...
export const ProductCRUD = () => dispatch => {
    ...
    axios.put(`https://african-marketplace-bw-1.herokuapp.com/api/inputs/`)
    .then(res => {
...
export const EditProductCRUD = edit => dispatch => {
    ...
    .then(res => {
        ...
        localStorage.setItem('token', res.data.token)
        dispatch({
            type: Put_Product_Success, payload: res
        })
    })
    .catch(err => {
        dispatch({
            type: Put_Product_Fail, payload: err
        })
    })
}
...
export const AddProductCRUD = add => dispatch => {
    ...
    .then(res => {
        ...
        localStorage.setItem('token', res.data.token)
        dispatch({
            type: Add_Product_Success, payload: res
        })
    })
    .catch(err => {
        dispatch({
            type: Add_Product_Fail, payload: err
        })
    })
}
